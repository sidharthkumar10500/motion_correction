...
#                         [ self.up_transpose_conv  for i in range(self.sites)])
        ...
        for site in range(self.sites):
            ...
            for i in range(num_pool_layers - 1):
                up_transpose_conv += [TransposeConvBlock(ch * 2, ch)]
                up_conv += [ConvBlock(ch * 2, ch, drop_prob)]
                ch //= 2
            ...
            self.Decoder_up_list.append(up_conv)

            self.Decoder_up_trans_list.append(up_transpose_conv)
...
class OldUnetModel(nn.Module):
    ...
    def forward(self, input):
        ...
        """
        stack = []
        output = input#input.unsqueeze(1)
